...
void files_tracker::prometheus_event_handler::report_prometheus_event(const struct files_event &e)
{
    for (size_t i = 0; i<e.rows;i++) {
        eunomia_files_write_counter
            .Add({
                { "bytes", std::to_string(e.values[i].write_bytes) },
            ...
            .Increment(e.values[i].writes);
        eunomia_files_read_counter
            .Add({ { "comm", std::string(e.values[i].comm) },
                { "filename", std::string(e.values[i].filename) },
                { "pid", std::to_string(e.values[i].pid) },
                { "bytes", std::to_string(e.values[i].read_bytes) },
                { "type", std::to_string(e.values[i].type) },
                }).Increment(e.values[i].reads);
    }
}
...
files_tracker::prometheus_event_handler::prometheus_event_handler(prometheus_server &server)
    : eunomia_files_read_counter(prometheus::BuildCounter()
                                     .Name("eunomia_observed_files_read")
                                     .Help("Number of observed files read")
                                     .Register(*server.registry)),
      eunomia_files_write_counter(prometheus::BuildCounter()
                                      .Name("eunomia_observed_files_write")
                                      .Help("Number of observed files write")
                                      .Register(*server.registry))
{
}
...
std::unique_ptr<files_tracker> files_tracker::create_files_tracker_with_default_env(files_event_handler handler)
{
  files_config config;
...
void files_tracker::start_tracker()
{
  ...
  start_file_tracker(
      handle_tracker_event<files_tracker, files_event>, libbpf_print_fn, current_config.env);
}

json files_tracker::json_event_handler::to_json(const struct files_event &e)
{
  std::string res;
  json files = {{ "type", "process" },
                         { "time", get_current_time() }};
  json files_event_json = json::array();
  for (size_t i = 0; i < e.rows; i++) {
      files_event_json.push_back({
                         { "pid", e.values[i].pid },
