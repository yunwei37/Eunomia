...
#define STARTS_WITH(mapname, prefix) (!strncmp(mapname, prefix, sizeof(prefix) - 1))
  ...
  return mapname[0] &&
         !(STARTS_WITH(mapname, "//anon") || STARTS_WITH(mapname, "/dev/zero") || STARTS_WITH(mapname, "/anon_hugepage") ||
           STARTS_WITH(mapname, "[stack") || STARTS_WITH(mapname, "/SYSV") || STARTS_WITH(mapname, "[heap]") ||
...
static int get_elf_text_scn_info(const char *path, uint64_t *addr, uint64_t *offset)
{
...
static struct dso *syms__find_dso(const struct syms *syms, unsigned long addr, uint64_t *offset)
{
...
static int dso__add_sym(struct dso *dso, const char *name, uint64_t start, uint64_t size)
{
  struct sym *sym;
  size_t new_cap;
  void *tmp;
...
static int dso__add_syms(struct dso *dso, Elf *e, Elf_Scn *section, size_t stridx, size_t symsize)
{
  Elf_Data *data = NULL;
...
static int dso__load_sym_table_from_elf(struct dso *dso, int fd)
{
  ...
  for (i = 0; i < dso->syms_sz; i++)
    dso->syms[i].name = btf__name_by_offset(dso->btf, (unsigned long)dso->syms[i].name);
...
struct syms *syms_cache__get_syms(struct syms_cache *syms_cache, int tgid)
{
  ...
  tmp = realloc(syms_cache->data, (syms_cache->nr + 1) * sizeof(*syms_cache->data));
  if (!tmp)
    return NULL;
  syms_cache->data = tmp;
...
static int partitions__add_partition(struct partitions *partitions, const char *name, unsigned int dev)
{
  struct partition *partition;
  void *tmp;

  tmp = realloc(partitions->items, (partitions->sz + 1) * sizeof(*partitions->items));
  if (!tmp)
    return -1;
...
struct partitions *partitions__load(void)
{
  ...
  while (fgets(buf, sizeof(buf), f) != NULL)
  {
    ...
    if (sscanf(buf, "%u %u %llu %s", &devmaj, &devmin, &nop, part_name) != 4)
      goto err_out;
    if (partitions__add_partition(partitions, part_name, MKDEV(devmaj, devmin)))
      goto err_out;
  }
