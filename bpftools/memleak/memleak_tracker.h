...
#ifndef MEMLEAK_TRACKER_H
#define MEMLEAK_TRACKER_H
...
static error_t parse_arg(int key, char *arg, struct argp_state *state)
{
  ...
  switch (key)
  {
    case 'p':
      ...
      env.pid = pid;
      break;
    case 't': env.trace_all = true; break;
    case 'a': env.show_allocs = true; break;
    case 'c': env.command = strdup(arg); break;
    case 'o':
      errno = 0;
...
static void
print_combined_outstanding_allocations(struct ksyms *ksyms, struct syms_cache *syms_cache, struct memleak_bpf *obj)
{
  ...
  printf(
      "[%02x:%02d:%02d] Top %d stacks with outstanding allocations:\n",
      timeinfo->tm_hour,
      timeinfo->tm_min,
      timeinfo->tm_sec,
      env.top);
...
static void print_outstanding_allocations(struct ksyms *ksyms, struct syms_cache *syms_cache, struct memleak_bpf *obj)
{
...
int start_memleak(int argc, char **argv)
{
