...
static const char *cap_name[] = { [0] = "CAP_CHOWN",
                                  [1] = "CAP_DAC_OVERRIDE",
...
static error_t parse_arg(int key, char *arg, struct argp_state *state)
{
  switch (key)
  {
    ...
    case 'c':
      ...
      env.cg = true;
      break;
    case 'U': env.user_stack = true; break;
    case 'K': env.kernel_stack = true; break;
    case 'x': env.extra_fields = true; break;
    case 'u':
      env.unique_type = arg;
      env.unique = true;
      break;
...
{
  struct cap_key_t lookup_key = {}, next_key;
...
{
  const struct cap_event *e = (const struct cap_event *)data;
  struct tm *tm;
  char ts[32];
  time_t t;
...
int start_capable(int argc, char **argv)
{
