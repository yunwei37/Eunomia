 ...
 */
#ifndef FUNCLATENCY_TRACKER_H
#define FUNCLATENCY_TRACKER_H
...
static int attach_uprobes(struct funclatency_bpf *obj)
{
  ...
  *function = '\0';
  ...
  obj->links.dummy_kretprobe =
      bpf_program__attach_uprobe(obj->progs.dummy_kretprobe, true, env.pid ?: -1, bin_path, func_off);
  if (!obj->links.dummy_kretprobe)
  {
    err = -errno;
...
int start_funclatency(int argc, char **argv)
{
